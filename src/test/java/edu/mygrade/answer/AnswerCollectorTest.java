...
class AnswerCollectorTest {
	...
	public void 입력한_순서대로_정답이_저장되어야_한다() {
		AnswerCollector answerCollector = new AnswerCollector(10);

		answerCollector.add(new Answer(5));
		answerCollector.add(new Answer(2));
		answerCollector.add(new Answer(3));
		answerCollector.add(new Answer(1));

		Iterator<Answer> iterator = answerCollector.iterator();

		assertEquals(iterator.next(), new Answer(5));
		assertEquals(iterator.next(), new Answer(2));
		assertEquals(iterator.next(), new Answer(3));
		assertEquals(iterator.next(), new Answer(1));
	}

	@Test
	@DisplayName("정답과 입력한 정답을 비교가 정상적으로 되어야 한다")
	public void 정답과_입력한_정답을_비교가_정상적으로_되어야_한다() {
		AnswerCollector inputCollector = new AnswerCollector(10);
		AnswerCollector answerCollector = new AnswerCollector(10);

		inputCollector.add(new Answer(3));
		inputCollector.add(new Answer(3));
		inputCollector.add(new Answer(2));
		inputCollector.add(new Answer(3));
		inputCollector.add(new Answer(2));
		inputCollector.add(new Answer(1));
		inputCollector.add(new Answer(1));
		inputCollector.add(new Answer(4));
		inputCollector.add(new Answer(3));
		inputCollector.add(new Answer(3));
	...
	public void 동일한_정답지나_정답지가_아닌_것과_비교할_경우_IllegalArgumentException_예외가_발생한다() {
		...
		assertThrows(IllegalArgumentException.class, () -> inputCollector.equalsAndGetIncorrectCount(new Answer(3)));
	}
